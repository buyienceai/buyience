...
8. **Approval routing.** When a requested price falls below the margin floor, the exception routes for approval from within the room, rather than pausing the negotiation for manual quote approvals while the rep tracks down a manager.
9. **Conversion to order.** The accepted terms complete the quote-to-order process directly, without manual re-entry — which removes a genuine source of error, since retyping an agreed quote into an order system is exactly where transcription mistakes happen.
